...
@description('Optional. The name of the subnet to create for the spoke application gateway for containers. If set, it overrides the name generated by the template.')
param spokeAG4CSubnetName string = 'snet-alb'

@description('CIDR of the spoke Application Gateway for Containers subnet. If the value is empty, this subnet will not be created.')
...
var infraProperties = (!empty(networkApplianceIpAddress))
  ? {
      addressPrefix: spokeInfraSubnetAddressPrefix
      networkSecurityGroup: {
        id: nsgAksEnvironment.outputs.nsgId
      }
      routeTable: {
...
var defaultSubnets = [
  ...
  {
    name: spokeAG4CSubnetName
    properties: {
      addressPrefix: spokeAG4CSubnetAddressPrefix
      networkSecurityGroup: {
        id: nsgApplicationGateway4Containers.outputs.nsgId
      }
      delegations: [
        {
          name: 'albdelegation'
...
module nsgAksEnvironment '../../../../shared/bicep/network/nsg.bicep' = {
...
module nsgAppGw '../../../../shared/bicep/network/nsg.bicep' =
  if (!empty(spokeApplicationGatewaySubnetAddressPrefix)) {
...
@description('NSG Rules for the alb subnet.')
module nsgApplicationGateway4Containers '../../../../shared/bicep/network/nsg.bicep' = {
  name: take('nsgAlb-${deployment().name}', 64)
  scope: spokeResourceGroup
  params: {
...
module egressLockdownUdr '../../../../shared/bicep/routeTables/main.bicep' =
  if (!empty(networkApplianceIpAddress)) {
...
module policyAssignments './modules/policy/policy-definition.module.bicep' =
  if (deployAzurePolicies) {
...
@description('The ID of the spoke private endpoints subnet.')
